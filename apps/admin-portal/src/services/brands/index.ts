...
type BrandsListResponse = {
  status: boolean;
  statusCode: number;
  data: { brands: BrandItem[]; pagination?: any };
};
...
export async function listBrands(
  ...
  const { data } = await with429Retry(() =>
    api.get<BrandsListResponse>("/brands/getAll", {
      params,
      signal: opts?.signal,
    })
  );
  return { rows: data?.data?.brands ?? [], pagination: data?.data?.pagination };
}

export async function getBrandById(id: string) {
  const { data } = await with429Retry(() =>
    api.get<BrandItemResponse>(`/brands/getById/${id}`)
  );
  return data.data;
}
...
export async function updateBrand(payload: {
  ...
  try {
    const { data } = await with429Retry(() =>
      api.put<BrandItemResponse>("/brands/update", payload)
    );
    return data.data;
  } catch {
    const { data } = await with429Retry(() =>
      api.post<BrandItemResponse>("/brands/update", payload)
    );
    return data.data;
  }
}
...
export async function deleteBrand(id: string) {
  const { data } = await with429Retry(() =>
    api.delete<BrandItemResponse>("/brands/delete", { data: { id } })
  );
  return data.data;
}
